...
class GlossaryLinkifier
{
    ...
    public const TOOL_COMPOUND_EXCLUSIONS = [
        ...
        // 2026-09-06 (revue Codex, ticket #2325) : les variantes VIRGULÉES sont déclarées ici,
        // une par une, au lieu du « ,? » global qui vivait dans le patron. Mesure qui l'impose :
        // ce « ,? » bloquait « Après Paragraph, Composer reste utile » - une énumération de DEUX
        // outils distincts, pas le composé « Paragraph Composer ». Il rendait aussi le lookbehind
        // à largeur VARIABLE, ce qui ne compile pas sous PCRE2 < 10.43 (cause de l'échec CI
        // rattrapé par 67952893 le jour même). Déclarer la virgule dans la donnée corrige les deux.
        'astra' => ['modèle', 'modele', 'model', 'models', 'that', 'synaptics',
                    'modèle,', 'modele,', 'model,', 'models,'],
    ];
    ...
    public const TOOL_SUFFIX_COMPOUND_EXCLUSIONS = [
        ...
        'haiku' => ['os'],
        // 2026-09-18 (fiche de glossaire « Hermes », mesure en production) : « Hermes Desktop »
        // est une fiche de l ANNUAIRE (l application de bureau de Nous Research) ; /glossaire/hermes
        // documente la NOTION. Sur la page de la fiche d annuaire elle-meme, une fiche ne s auto-lie
        // pas : la forme longue cesse d etre candidate, le tri par longueur decroissante n a plus
        // rien a departager, et le NOM court gagne par defaut - 5 liens « Hermes » laissant
        // « Desktop » en texte nu. Le retrait de l ALIAS homonyme n y changeait RIEN, puisque le
        // lien vient du NOM du terme. Meme mecanique que « Haiku OS » juste au-dessus.
        'hermes' => ['desktop'],
    ];
                            ...
                            if (is_array($aliases)) {
                                foreach ($aliases as $alias) {
                                    ...
                                    $terms[] = [
                                        ...
                                        // 2026-09-05 (ticket #2241) : l'exclusion de suffixe vivait sur le NOM
                                        // du terme seulement. Un ALIAS n'en portait aucune - or « Haiku » est le
                                        // PREMIER alias de sa fiche, donc c'est cette entree-ci qui posait le faux
                                        // lien sur « Haiku OS ». Meme famille que #2137 : l'angle mort est l'alias.
                                        'exclude_suffix' => self::TOOL_SUFFIX_COMPOUND_EXCLUSIONS[mb_strtolower(trim($alias))] ?? [],
                                    ];
                                }
                            }
                            ...
                            foreach (self::extractQualifierAliases($name) as $autoAlias) {
                                ...
                                $terms[] = [
                                    ...
                                    'origin_rank' => self::ORIGIN_DERIVED_ALIAS,
                                        ...
                                        // PREMIER alias de sa fiche, donc c'est cette entree-ci qui posait le faux
                                        // lien sur « Haiku OS ». Meme famille que #2137 : l'angle mort est l'alias.
                                        'exclude_suffix' => self::TOOL_SUFFIX_COMPOUND_EXCLUSIONS[mb_strtolower(trim($autoAlias))] ?? [],
                                ];
                            }
                            ...
                            foreach (array_merge([$morphoBase], self::extractQualifierAliases($name)) as $candidate) {
                                foreach (self::extractMorphologicalAliases($candidate) as $morpho) {
                                    ...
                                    $terms[] = [
                                        ...
                                        'origin_rank' => self::ORIGIN_DERIVED_ALIAS,
                                        // 2026-09-05 (ticket #2241) : 3e et dernier bloc d'alias de glossaire.
                                        'exclude_suffix' => self::TOOL_SUFFIX_COMPOUND_EXCLUSIONS[mb_strtolower(trim($morpho))] ?? [],
                                    ];
                                }
                            }
                        });
    ...
    public static function flushCache(): void
    {
        ...
        foreach (['fr_CA', 'fr', 'en', 'en_CA'] as $loc) {
            ...
            Cache::forget('glossary.terms.v24.'.$loc);
