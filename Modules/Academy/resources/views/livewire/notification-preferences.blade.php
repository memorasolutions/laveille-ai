        ...
        @foreach (\Modules\Academy\Services\AcademyNotificationService::TYPES as $type)
            ...
            <li style="display:flex;justify-content:space-between;align-items:center;gap:16px;padding:16px 0;border-bottom:1px solid var(--sys-border, #e5e7eb);">
                ...
                <button
                    ...
                    style="flex:none;min-width:64px;min-height:32px;border:1px solid {{ $on ? '#0d9488' : '#cbd5e1' }};border-radius:9999px;background-color:{{ $on ? '#0d9488' : '#e2e8f0' }};color:{{ $on ? '#ffffff' : '#475569' }};font-size:13px;font-weight:600;cursor:pointer;padding:6px 12px;">
                    {{ $on ? 'Activé' : 'Désactivé' }}
                </button>
            </li>
        @endforeach
