...
@section('title', __('Modifier l\'incident'))
...
<div class="card">
    <div class="card-header">
        <h5 class="mb-0">{{ __('Modifier l\'incident') }}</h5>
    </div>
    <div class="card-body">
        <form action="{{ route('admin.health.incidents.update', $incident) }}" method="POST">
            @csrf
            @method('PUT')
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('status') is-invalid @enderror" id="status" name="status">
                            <option value="investigating" {{ old('status', $incident->status) == 'investigating' ? 'selected' : '' }}>{{ __('Investigation') }}</option>
                            <option value="identified" {{ old('status', $incident->status) == 'identified' ? 'selected' : '' }}>{{ __('Identifié') }}</option>
                            <option value="monitoring" {{ old('status', $incident->status) == 'monitoring' ? 'selected' : '' }}>{{ __('Surveillance') }}</option>
                            <option value="resolved" {{ old('status', $incident->status) == 'resolved' ? 'selected' : '' }}>{{ __('Résolu') }}</option>
                        </select>
                        @error('status')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('severity') is-invalid @enderror" id="severity" name="severity">
                            <option value="minor" {{ old('severity', $incident->severity) == 'minor' ? 'selected' : '' }}>{{ __('Mineur') }}</option>
                            <option value="major" {{ old('severity', $incident->severity) == 'major' ? 'selected' : '' }}>{{ __('Majeur') }}</option>
                            <option value="critical" {{ old('severity', $incident->severity) == 'critical' ? 'selected' : '' }}>{{ __('Critique') }}</option>
                        </select>
                        @error('severity')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
