...
@extends('backoffice::themes.backend.layouts.admin', ['title' => __('Modifier le coupon'), 'subtitle' => __('Boutique')])
...
<nav class="page-breadcrumb" aria-label="Fil d'Ariane">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item"><a href="{{ route('admin.ecommerce.coupons.index') }}">{{ __('Coupons') }}</a></li>
                ...
                <form action="{{ route('admin.ecommerce.coupons.update', $coupon) }}" method="POST">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label class="form-label">{{ __('Code promo') }}</label>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="type" id="couponType" class="form-select">
                                <option value="fixed" {{ old('type', $coupon->type) === 'fixed' ? 'selected' : '' }}>{{ __('Montant fixe') }}</option>
                                <option value="percent" {{ old('type', $coupon->type) === 'percent' ? 'selected' : '' }}>{{ __('Pourcentage (%)') }}</option>
                                <option value="free_shipping" {{ old('type', $coupon->type) === 'free_shipping' ? 'selected' : '' }}>{{ __('Livraison gratuite') }}</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="number" name="max_uses" class="form-control" value="{{ old('max_uses', $coupon->max_uses) }}" placeholder="{{ __('Illimité') }}">
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-6 mb-3">
