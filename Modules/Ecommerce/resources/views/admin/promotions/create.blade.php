...
@extends('backoffice::themes.backend.layouts.admin', ['title' => __('Nouvelle promotion'), 'subtitle' => __('Boutique')])
...
@section('content')
    ...
    <form action="{{ route('admin.ecommerce.promotions.store') }}" method="POST">
        @csrf
        <div class="card">
            ...
            <div class="card-body">
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select" id="type" name="type" required>
                            <option value="percentage_off" {{ old('type') == 'percentage_off' ? 'selected' : '' }}>{{ __('Remise en pourcentage') }}</option>
                            <option value="fixed_off" {{ old('type') == 'fixed_off' ? 'selected' : '' }}>{{ __('Remise fixe') }}</option>
                            <option value="bogo" {{ old('type') == 'bogo' ? 'selected' : '' }}>{{ __('1 acheté = 1 offert') }}</option>
                            <option value="free_shipping" {{ old('type') == 'free_shipping' ? 'selected' : '' }}>{{ __('Livraison gratuite') }}</option>
                            <option value="tiered_pricing" {{ old('type') == 'tiered_pricing' ? 'selected' : '' }}>{{ __('Prix par palier') }}</option>
                        </select>
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select" id="applies_to" name="applies_to">
                            <option value="all" {{ old('applies_to', 'all') == 'all' ? 'selected' : '' }}>{{ __('Tout') }}</option>
                            <option value="specific_products" {{ old('applies_to') == 'specific_products' ? 'selected' : '' }}>{{ __('Produits spécifiques') }}</option>
                            <option value="specific_categories" {{ old('applies_to') == 'specific_categories' ? 'selected' : '' }}>{{ __('Catégories spécifiques') }}</option>
                        </select>
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="number" class="form-control" id="max_uses" name="max_uses" value="{{ old('max_uses') }}" placeholder="{{ __('Illimité') }}">
                ...
                <div class="row mb-3">
                    <div class="col-md-6 d-flex align-items-center gap-4">
                        ...
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" id="is_stackable" name="is_stackable" value="1" {{ old('is_stackable') ? 'checked' : '' }}>
                            <label class="form-check-label" for="is_stackable">{{ __('Cumulable') }}</label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" id="is_automatic" name="is_automatic" value="1" {{ old('is_automatic', true) ? 'checked' : '' }}>
                ...
                <div class="mt-4">
                    <button type="submit" class="btn btn-primary">
                        <i data-lucide="save" class="me-1"></i> {{ __('Créer') }}
                    </button>
