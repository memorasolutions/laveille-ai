...
@section('content')
    <div class="mx-auto max-w-2xl">
        <h2 class="mb-6 text-xl font-bold text-gray-900 dark:text-white">Modifier {{ $user->name }}</h2>

        <form method="POST" action="{{ route('admin.users.update', $user) }}" class="space-y-6 rounded-xl bg-white p-6 shadow-sm ring-1 ring-gray-950/5 dark:bg-gray-800 dark:ring-white/10">
            @csrf
            @method('PUT')
            ...
            <div>
                <label for="password" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Nouveau mot de passe <span class="text-gray-400">(laisser vide pour ne pas changer)</span></label>
            ...
            <div>
                <label class="flex items-center gap-2">
                    ...
                    <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Forcer le changement de mot de passe au prochain login</span>
                </label>
            </div>
            ...
            <div>
                ...
                <div class="mt-2 space-y-2">
                    @foreach($roles as $id => $name)
                        <label class="flex items-center gap-2">
                            <input type="checkbox" name="roles[]" value="{{ $id }}" {{ $user->hasRole($name) ? 'checked' : '' }}
                                   class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 dark:border-gray-600">
