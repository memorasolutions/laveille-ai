...
@extends('backoffice::themes.backend.layouts.admin', ['title' => __('Rôles'), 'subtitle' => __('Modifier')])
...
<div class="d-flex align-items-center justify-content-between flex-wrap gap-3 mb-3">
    <h4 class="fw-bold mb-0 d-flex align-items-center gap-2"><i data-lucide="shield-check" class="icon-md text-primary"></i>{{ __('Modifier le rôle :') }} {{ $role->name }}</h4>
...
<form action="{{ route('admin.roles.update', $role) }}" method="POST" x-data="permissionsManager()">
    @csrf
    @method('PUT')
            ...
            <div class="row">
                ...
                <div class="col-md-6 d-flex align-items-end">
                    <div class="form-check form-switch">
                        <input class="form-check-input" type="checkbox" name="requires_password" value="1" id="requires_password"
                               {{ old('requires_password', $role->requires_password ?? true) ? 'checked' : '' }}>
...
function permissionsManager() {
    const rolePermIds = @json($role->permissions->pluck('id')->map(fn($id) => (int) $id));
    const oldPerms = @json(old('permissions', []));
    const hasOld = oldPerms.length > 0;
    const activePerms = hasOld ? oldPerms.map(Number) : rolePermIds;
    ...
    Object.entries(permMap).forEach(([name, meta]) => {
        checked[name] = activePerms.includes(meta.id);
