...
@extends('backoffice::themes.backend.layouts.admin', ['title' => __('Import CSV'), 'subtitle' => __('Abonnés')])
...
<nav class="page-breadcrumb" aria-label="Fil d'Ariane">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item"><a href="{{ route('admin.newsletter.index') }}">{{ __('Newsletter') }}</a></li>
    ...
    <div class="card-header py-3 px-4 border-bottom">
        <div class="d-flex align-items-center justify-content-between flex-wrap gap-3">
            <h4 class="fw-bold mb-0 d-flex align-items-center gap-2">
                ...
                {{ __('Importer des abonnés depuis un fichier CSV') }}
            </h4>
            <a href="{{ route('admin.newsletter.index') }}" class="btn btn-light btn-sm d-inline-flex align-items-center gap-2">
                <i data-lucide="arrow-left" class="icon-sm"></i>
                {{ __('Retour aux abonnés') }}
            </a>
        </div>
    </div>
    <div class="card-body p-4">
        ...
        <div class="alert alert-info d-flex align-items-start gap-3 mb-4">
            ...
            <div>
                ...
                <p class="mb-1">{{ __('Colonnes dans cet ordre :') }} <code class="bg-primary bg-opacity-10 px-2 py-1 rounded small">email, name</code></p>
                <p class="mb-1">{{ __("La 1ère ligne (en-tête) est ignorée. L'email sert d'identifiant unique.") }}</p>
                <p class="mb-0">{{ __('La colonne') }} <code class="bg-primary bg-opacity-10 px-2 py-1 rounded small">name</code> {{ __("est optionnelle. Les abonnés sont confirmés automatiquement à l'import.") }}</p>
            </div>
        </div>
