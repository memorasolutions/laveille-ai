@extends('backoffice::layouts.admin', ['title' => 'Utilisateurs', 'subtitle' => 'Modifier'])

@section('content')

<div class="card">
    <div class="card-header">
        <h6 class="mb-0">Modifier l'utilisateur : {{ $user->name }}</h6>
    </div>
    <div class="card-body">
        <form action="{{ route('admin.users.update', $user) }}" method="POST">
            ...
            <div class="row gy-3">
                ...
                <div class="col-md-6">
                    <label class="form-label fw-semibold text-primary-light text-sm mb-8">Nouveau mot de passe <span class="text-secondary-light">(laisser vide pour ne pas changer)</span></label>
                    ...
                    <div class="d-flex flex-wrap gap-3">
                        @foreach($roles as $id => $name)
                            <div class="form-check d-flex align-items-center" style="gap: 0.5rem;">
                                <input class="form-check-input mt-0" type="checkbox" name="roles[]" value="{{ $id }}" id="role_{{ $id }}" @checked(in_array($id, old('roles', $user->roles->pluck('id')->toArray())))>
