...
@section('page-title', 'Modifier ' . $role->name)

@section('content')
    <div class="mx-auto max-w-2xl">
        <h2 class="mb-6 text-xl font-bold text-gray-900 dark:text-white">Modifier {{ $role->name }}</h2>

        <form method="POST" action="{{ route('admin.roles.update', $role) }}" class="space-y-6 rounded-xl bg-white p-6 shadow-sm ring-1 ring-gray-950/5 dark:bg-gray-800 dark:ring-white/10">
            @csrf
            @method('PUT')
            ...
            <div>
                <label class="flex items-center gap-2">
                    ...
                    <input type="checkbox" name="requires_password" value="1" {{ old('requires_password', $role->requires_password ?? true) ? 'checked' : '' }}
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-2 gap-2 sm:grid-cols-3">
                                @foreach($perms as $perm)
                                    <label class="flex items-center gap-2">
                                        <input type="checkbox" name="permissions[]" value="{{ $perm->id }}" {{ $role->hasPermissionTo($perm->name) ? 'checked' : '' }}
                                               class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 dark:border-gray-600">
