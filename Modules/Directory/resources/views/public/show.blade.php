...
@include('core::components.admin-bar', [
    ...
    'actions' => array_filter([
        ...
        Route::has('admin.directory.tool.toggle-featured') ? ['label' => $tool->isSponsored() ? __('Désactiver sponsorisé') : __('Activer sponsorisé (30j)'), 'icon' => 'star', 'url' => route('admin.directory.tool.toggle-featured', $tool->id), 'method' => 'POST', 'confirm' => $tool->isSponsored() ? __('Désactiver le sponsoring pour cet outil ?') : __('Activer le sponsoring pour 30 jours ?')] : null,
        ['divider' => true],
        ...
        <div style="display: flex; align-items: flex-start; gap: 16px; flex-wrap: wrap;">
            ...
            <div style="flex: 1; min-width: 200px;">
                <div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap;">
                    ...
                    <x-core::sponsored-badge :tool="$tool" size="md" />
            ...
            <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 16px; padding: 24px; margin-top: 24px;"
                ...
                <div x-show="step===2" x-transition role="region" aria-label="Étape 2 : URL de la ressource">
                    ...
                    <div style="display:flex;gap:8px;margin-top:16px;">
                        ...
                        <button type="button" @click="if (!/^https?:\/\/[^\s]+\.[^\s]+/i.test(url)) { window.dispatchEvent(new CustomEvent('toast-show', { detail: { message: 'URL invalide. Utilisez https:// avec un domaine valide.', variant: 'warning', duration: 4000 } })); return; } fetchMeta()" :disabled="!url || url.length < 10 || !/^https?:\/\/[^\s]+\.[^\s]+/i.test(url)" style="padding:10px 20px;border:none;border-radius:8px;background:var(--c-primary);color:#fff;cursor:pointer;font-weight:600;flex:1;" :style="(!url || url.length < 10 || !/^https?:\/\/[^\s]+\.[^\s]+/i.test(url)) && 'opacity:0.5;cursor:not-allowed'">{{ __('Continuer') }} →</button>
                    </div>
                </div>
