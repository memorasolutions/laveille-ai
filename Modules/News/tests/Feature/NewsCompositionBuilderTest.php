...
// La PRÉSENCE de news-article-picker.js et de l'appel compositionBuilder(...) dans le HTML ne
// prouve rien : les deux étaient déjà présents AVANT le correctif, et l'écran cascadait quand
// même en ~34 ReferenceError (le script chargeait APRÈS que Livewire ait déjà démarré Alpine et
// évalué x-data). La preuve porte sur la POSITION RELATIVE dans le HTML rendu, traduction directe
// de « le script arrive avant le code qui s'en sert » et de ce que @assets (voir
// news::admin.partials.news-article-picker) est censé garantir : injection dans <head>, avant
// x-data (dans <body>) ET avant le script Livewire qui démarre Alpine (@livewireScripts, en fin
// de <body> - Modules/Backoffice/.../layouts/admin.blade.php:180).
it('news-article-picker.js charge avant l\'appel compositionBuilder(...) et avant le script Livewire qui démarre Alpine', function () {
    $admin = ncbAdmin();

    $html = $this->actingAs($admin)->get(route('admin.news.composition.index'))->getContent();

    $scriptPos = strpos($html, 'news-article-picker.js');
    $factoryCallPos = strpos($html, 'compositionBuilder(');
    $livewireBootPos = strpos($html, 'livewire.js');

    expect($scriptPos)->not->toBeFalse('news-article-picker.js absent du HTML rendu');
    expect($factoryCallPos)->not->toBeFalse('appel compositionBuilder(...) absent du HTML rendu');
...
it('ne laisse aucun buffer de sortie ouvert après le rendu de l\'écran de composition', function () {
    $admin = ncbAdmin();
