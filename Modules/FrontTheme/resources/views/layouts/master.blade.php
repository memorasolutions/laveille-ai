...
@include('fronttheme::partials.newsletter-sticky-bar')
    ...
    @php
        ...
        // Texte de partage PAR RÉSEAU (corrige l'incohérence X constatée le 2026-08-20 :
        // l'intent et le presse-papiers utilisaient deux textes différents). Une section Blade
        // dédiée par réseau (share_text_x/linkedin/facebook/messenger, ex. News::show) prime ;
        // à défaut, repli sur le texte générique existant pour ne rien changer aux autres
        // gabarits (Blog, glossaire, etc.) qui ne les définissent pas encore.
        $shareVariant = function (string $section, string $fallback) use ($__env) {
            if (! $__env->hasSection($section)) {
                return $fallback;
            }
            $raw = trim($__env->yieldContent($section) ?: '');

            return $raw !== '' ? html_entity_decode($raw, ENT_QUOTES, 'UTF-8') : '';
        };
        $shareTexts = [
    ...
    @endphp
    {{-- 2026-08-21 : &quote= (Facebook) et summary= (LinkedIn) retirés des deux liens d'intent
         ci-dessous (sidebar desktop + barre mobile). Vérifié sur la documentation officielle :
         Meta interdit le préremplissage du texte de partage et ignore le paramètre `quote` sur
         sharer.php ; LinkedIn a déprécié `summary` sur cet endpoint. Ces paramètres étaient donc
         inertes, et l'un des deux allait à l'encontre de la politique de la plateforme. Le texte
         continue de vivre par le presse-papiers (window.__openShare copie shareTexts.* avant
         d'ouvrir l'intent) - aucune perte fonctionnelle. --}}
    <div x-data="{ copied: false, linkedinCopied: false, showLiModal: false, liUrl: '', liText: {{ \Illuminate\Support\Js::from($shareClipboard) }}, shareTexts: { fb: {{ \Illuminate\Support\Js::from($shareTexts['facebook']) }}, x: {{ \Illuminate\Support\Js::from($shareTexts['x']) }}, li: {{ \Illuminate\Support\Js::from($shareTexts['linkedin']) }}, msg: {{ \Illuminate\Support\Js::from($shareTexts['messenger']) }} }, openLi() { window.open(this.liUrl, '_blank'); this.showLiModal = false; } }" class="share-float">
        {{-- Desktop sidebar --}}
        <div class="share-sidebar">
            <a href="https://www.facebook.com/sharer/sharer.php?u={{ $shareUrl }}" target="_blank" rel="noopener" aria-label="{{ __('Partager sur Facebook') }}" class="share-btn share-fb" @click.prevent="window.__openShare($el.href,shareTexts.fb,'Facebook','#1877F2')">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
            </a>
            <a href="https://twitter.com/intent/tweet?url={{ $shareUrl }}&text={{ urlencode($shareTexts['x']) }}" target="_blank" rel="noopener" aria-label="{{ __('Partager sur X') }}" class="share-btn share-x" @click.prevent="window.__openShare($el.href,shareTexts.x,'X','#000')">
            ...
            <a href="https://www.linkedin.com/shareArticle?mini=true&url={{ $shareUrl }}" target="_blank" rel="noopener" aria-label="{{ __('Partager sur LinkedIn') }}" class="share-btn share-li" @click.prevent="window.__openLinkedIn($el.href,shareTexts.li)">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
            </a>
            <a href="{{ lv_social('messenger') }}" target="_blank" rel="noopener" aria-label="{{ __('Envoyer via Messenger') }}" class="share-btn share-msg" @click.prevent="window.__openShare($el.href,shareTexts.msg,'Messenger','#0099FF','message')">
            ...
            <button @click="(async()=>{const txt=(liText && liText.trim()) ? liText : window.location.href; await navigator.clipboard.writeText(txt); copied=true; setTimeout(()=>copied=false,2000);})()" :aria-label="copied ? '{{ __('Contenu copié') }}' : '{{ __('Copier le contenu enrichi') }}'" class="share-btn share-copy">
        ...
        <div class="share-bottom">
            <a href="https://www.facebook.com/sharer/sharer.php?u={{ $shareUrl }}" target="_blank" rel="noopener" aria-label="{{ __('Partager sur Facebook') }}" class="share-btn share-fb" @click.prevent="window.__openShare($el.href,shareTexts.fb,'Facebook','#1877F2')"><svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg></a>
            <a href="https://twitter.com/intent/tweet?url={{ $shareUrl }}&text={{ urlencode($shareTexts['x']) }}" target="_blank" rel="noopener" aria-label="{{ __('Partager sur X') }}" class="share-btn share-x" @click.prevent="window.__openShare($el.href,shareTexts.x,'X','#000')"><svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg></a>
            <a href="https://www.linkedin.com/shareArticle?mini=true&url={{ $shareUrl }}" target="_blank" rel="noopener" aria-label="{{ __('Partager sur LinkedIn') }}" class="share-btn share-li" @click.prevent="window.__openLinkedIn($el.href,shareTexts.li)"><svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg></a>
            <button @click="(async()=>{const txt=(liText && liText.trim()) ? liText : window.location.href; await navigator.clipboard.writeText(txt); copied=true; setTimeout(()=>copied=false,2000);})()" :aria-label="copied ? '{{ __('Contenu copié') }}' : '{{ __('Copier le contenu enrichi') }}'" class="share-btn share-copy">
                <svg x-show="!copied" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1"/></svg>
                <svg x-show="copied" x-cloak width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#10b981" stroke-width="2.5" aria-hidden="true"><polyline points="20 6 9 17 4 12"/></svg>
            </button>
        </div>
