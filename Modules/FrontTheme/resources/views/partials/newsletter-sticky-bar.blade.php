<div id="newsletter-sticky-bar"
    ...
    <div style="max-width: 1200px; margin: 0 auto; padding: 0 16px; display: flex; align-items: center; justify-content: space-between; height: 100%; gap: 12px;">
        <div style="display: flex; align-items: center; gap: 8px; min-width: 0;">
            <span aria-hidden="true" style="color: #064E5A; font-size: 14px; line-height: 1;">▸</span>
            <span class="sticky-badge" style="background: #E0F2FE; color: #064E5A; font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: 1px; padding: 2px 6px; border-radius: 3px; white-space: nowrap;">{{ __('Veille IA') }}</span>
        ...
        <div style="display: flex; align-items: center; gap: 16px; flex-shrink: 0;">
            <button type="button" data-bs-toggle="modal" data-bs-target="#newsletterModal" aria-label="{{ __('Ouvrir le formulaire d\'inscription à l\'infolettre hebdomadaire') }}" style="color: #064E5A; background: none; border: none; padding: 4px 0; min-height: 28px; font-size: 13px; font-weight: 600; text-decoration: underline; text-underline-offset: 2px; cursor: pointer; transition: color 150ms ease;" onmouseover="this.style.color='#9A2A06'" onmouseout="this.style.color='#064E5A'">{{ __('Recevoir hebdo →') }}</button>
            <button type="button" @click="dismissed = true; localStorage.setItem('newsletter_sticky_dismissed_at', Date.now().toString());" aria-label="{{ __('Fermer le bandeau infolettre') }}" style="background: none; border: none; color: #1F2937; font-size: 18px; line-height: 1; cursor: pointer; padding: 4px 8px; min-width: 28px; min-height: 28px; transition: color 150ms ease;" onmouseover="this.style.color='#9A2A06'" onmouseout="this.style.color='#1F2937'">&times;</button>
        </div>
    </div>
</div>
<style>
