...
it('confirms the diagnostic panel cannot serve as the fallback (round 131 root cause)', function () {
    ...
    // (2026-08-05, disclosure « Vérifications » ex-« Diagnostic rapide ») puis au correctif étape
    // prématurée (2026-08-06) : isValid reste la condition racine (donc toujours inutilisable comme
    // fallback avant que le formulaire soit valide), avec en plus un filtre sur les suggestions
    // applicables à l'étape courante pour ne jamais ouvrir le panneau vide.
    expect($blade)->toContain('<div class="ct-disclosure" x-show="isValid && (diagnostic.issues.length > 0 || (diagnostic.ok && step === 4))" x-cloak>');
});
