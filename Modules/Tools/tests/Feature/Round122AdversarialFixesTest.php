...
it('restores card focus on both duplicate failure paths only (round 122)', function () {
    ...
    expect($blade)->toContain('_restoreCardFocus(publicId) {');
    expect($blade)->toContain("card.querySelector('[x-ref=trigger]')");
    ...
    expect(substr_count($blade, '_restoreCardFocus(publicId);'))->toBe(2);
    expect($blade)->toContain('window.location.reload();');
});
...
it('guards the restore behind an actual focus loss (round 122, no side effect)', function () {
    ...
    expect($blade)->toContain('if (document.activeElement !== document.body) return;');
});
