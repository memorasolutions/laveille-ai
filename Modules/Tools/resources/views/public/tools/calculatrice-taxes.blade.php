                                ...
                                <section class="main-calculator card">
                                    <div class="form-group">
                                        <label for="province">
                                            ...
                                            <button type="button" class="ct-help-btn" data-help-key="province" aria-label="{{ __('Aide pour Province') }}">ⓘ</button>
                                        </label>
                                    ...
                                    <p style="font-size: 0.85rem; color: var(--c-text-muted, #52586a); margin: 0 0 0.75rem 0; padding: 0.5rem 0.75rem; background: #f1f3f5; border-radius: 8px; border-left: 3px solid var(--c-primary, #064E5A);">
                                        💡 {{ __('Saisissez le montant avant OU avec taxes — l\'autre champ se calcule automatiquement.') }}
                                    </p>
                    ...
                    <div class="calculator-grid">
                                        <div class="form-group">
                                            <label for="amount-before-tax">
                                                ...
                                                <button type="button" class="ct-help-btn" data-help-key="avant_taxes" aria-label="{{ __('Aide pour Montant avant taxes') }}">ⓘ</button>
                                            </label>
                                        ...
                                        <div class="form-group">
                                            <label for="amount-after-tax">
                                                ...
                                                <button type="button" class="ct-help-btn" data-help-key="avec_taxes" aria-label="{{ __('Aide pour Montant avec taxes') }}">ⓘ</button>
                                            </label>
                                            <div class="input-wrapper">
                                                <span class="currency-symbol">$</span>
...
@push('scripts')
<script>
    {{-- #16 S84 : contenu aide popup centralisé pour éviter escapes apostrophes Blade dans @click --}}
    window.HELP_CONTENT = @json([
        'province' => [
            'title' => 'ⓘ ' . __('Province / Territoire'),
            'body' => '<p>' . __('Choisissez votre province ou territoire canadien. Les taxes varient selon la région :') . '</p>'
                    . '<ul>'
                    . '<li><strong>' . __('TPS (5 %) seule') . '</strong> : Alberta, Yukon, Territoires du Nord-Ouest, Nunavut.</li>'
                    . '<li><strong>' . __('TPS + TVQ') . '</strong> : ' . __('Québec (5 % + 9,975 % = 14,975 %).') . '</li>'
                    . '<li><strong>' . __('TPS + TVP') . '</strong> : ' . __('Colombie-Britannique (12 %), Manitoba (12 %), Saskatchewan (11 %).') . '</li>'
                    . '<li><strong>' . __('TVH unique') . '</strong> : ' . __('Ontario (13 %), Nouveau-Brunswick / Terre-Neuve / Île-du-Prince-Édouard (15 %), Nouvelle-Écosse (14 %).') . '</li>'
                    . '</ul>'
                    . '<p style="font-size:0.85rem; color: var(--c-text-muted, #52586a);">' . __('Taux mis à jour 2025 selon Revenu Canada et Revenu Québec.') . '</p>',
        ],
        'avant_taxes' => [
            'title' => 'ⓘ ' . __('Montant avant taxes'),
            'body' => '<p>' . __('Le montant <strong>avant taxes</strong> (sous-total HT) est le prix affiché de l’article ou du service, <strong>avant ajout</strong> de la TPS, TVQ, TVP ou TVH.') . '</p>'
                    . '<p><strong>' . __('Exemple Québec :') . '</strong> ' . __('vous achetez un article à 100 $ avant taxes. À la caisse, on ajoute 5 $ TPS et 9,98 $ TVQ. Vous payez 114,98 $ au total.') . '</p>'
                    . '<p style="font-size:0.85rem; color: var(--c-text-muted, #52586a);">💡 ' . __('Saisissez ce montant si vous connaissez le prix avant taxes — l’autre champ se calcule automatiquement.') . '</p>',
        ],
        'avec_taxes' => [
            'title' => 'ⓘ ' . __('Montant avec taxes'),
            'body' => '<p>' . __('Le montant <strong>avec taxes</strong> (TTC) est le total final que vous payez, <strong>incluant</strong> toutes les taxes applicables.') . '</p>'
                    . '<p><strong>' . __('Exemple Québec :') . '</strong> ' . __('vous payez 114,98 $ à la caisse. La calculatrice décompose : 100 $ avant taxes + 5 $ TPS + 9,98 $ TVQ.') . '</p>'
                    . '<p style="font-size:0.85rem; color: var(--c-text-muted, #52586a);">💡 ' . __('Saisissez ce montant pour faire le calcul inversé — l’autre champ se calcule automatiquement.') . '</p>'
                    . '<p style="font-size:0.85rem; color: var(--c-text-muted, #52586a);">🍽️ ' . __('Si vous avez ajouté un pourboire à ce montant, cochez « Le total inclut un pourboire » pour décomposer pourboire, taxes et sous-total.') . '</p>',
        ],
    ]) ;
    window.taxConfig = {
        tax_rates: {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // #16 S84 v3 : Bidirectionnel natif via engine activeField. Toggle pourboire optionnel s'applique au champ saisi 'avec taxes'.
    ...
    // Helper DRY : extrait données calcul actuel du DOM (source-aware via simpleCalculator.state.activeField)
    ...
    // Construire URL deep-link (#15 + #16 v3 — rétrocompat ancien ?m=)
    ...
    // #16 S84 v3 : Bidirectionnel natif. Toggle pourboire toujours visible — actif si user saisit dans 'avec taxes'.
    ...
    // Override : si tipIncluded actif ET user saisit dans 'after', recompute reverse_tip et écrase engine
    ...
    // Si user repasse en saisie 'before', cache le breakdown pourboire
