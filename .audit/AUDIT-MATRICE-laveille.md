...
| hygiene-serveur | complété | **Blocage levé le 2026-08-26** : les crons de production ont été lus. 4 tâches pour laveille.ai, toutes légitimes, **aucun résidu**. Un défaut y a été trouvé et corrigé : le worker de file `cloudflare,screenshots,news-tools,workflows` n'avait aucun `--timeout` (défaut 60 s) et tuait les captures. Local : `opcache_reset.php` et `audit-console-errors.log` résiduels. Prod par HTTP : aucun fichier sensible exposé (10 chemins testés). |
