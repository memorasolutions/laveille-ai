...
### Ajouté
- **Annuaire : la file des vignettes sans marge de recadrage se résorbe toute seule, la nuit** - nouvelle planification `directory:dispatch-margin-recapture --limit=150 --restart-every=0` à 02h05 Québec (fuseau forcé explicitement par `->timezone('America/Toronto')`, pour ne pas dépendre de la configuration d'environnement), `withoutOverlapping()` et `runInBackground()`, calquée sur la planification voisine de `directory:capture-screenshots`. Ticket #2087.

  **Pourquoi 150 et non les ~551 restants d'un coup**, alors que le plan écrit disait « lancer les ~551 restants » : le worker de la file `screenshots` digère environ 1,2 job à la minute (mesure du lot 1, 100 jobs entre 04h02 et 04h32 Québec). À ce rythme, 551 jobs occupent plus de sept heures et retombent en pleine journée, bien au-delà de la fenêtre calme - c'est exactement la contention qui a rendu la moitié des pages du site indisponibles le 31 août (#2100, #2107). La fenêtre réellement libre va de 02h05 à 04h30, où `directory:capture-screenshots` prend le relais : environ 2 h 25, soit ~174 jobs. 150 tient dedans avec de la marge, et la file s'épuise en quatre nuits au lieu de menacer une journée de service.

  Effet de bord voulu : le filtre anti-futiles livré en v1.248.1 écarte 30 jours un outil déjà tenté sans succès, puis le retente. Cette planification devient donc un entretien continu plutôt qu'une opération unique - un site protégé par un CAPTCHA aujourd'hui redevient candidat dans un mois, sans intervention.
...
- **Preuve en production après déploiement (footer confirmé v1.245.3).** Fiches battery/véhicule `alerte-prix-fou-le-pack-google-pixel-10-et-jbl-flip-7-perd-369-eur-chez-darty` et `motorola-edge-70-fusion-le-monstre-dautonomie-7-000-mah-chute-deja-a-429-eur-chez-boulanger` : lien `/glossaire/autonomie-ia` disparu (0 occurrence, HTTP 200). Deux des 9 survivantes vérifiées vivantes : `as-ai-agents-take-on-more-tasks-governance-becomes-a-priority` (ancre « IA autonomes ») et `nvidia-vera-quand-le-cpu-devient-le-cerveau-de-lia-autonome` (ancre « autonomie des machines ») pointent toujours vers `/glossaire/autonomie-ia`. Contre-exemple exact du compromis assumé : `anthropic-veut-amener-claude-a-un-niveau-de-perpetuelle-autonomie` (fiche #12983, citée en v1.244.14 comme lien légitime qui fonctionnait encore) a bien perdu son lien - bare « autonomie », aucun des 8 alias curés. Page du terme (`/glossaire/autonomie-ia`) toujours HTTP 200.
