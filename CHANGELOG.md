...
- **Périmètre.** Le même motif `{{ e(...) }}` existe aussi, indépendamment, dans les meta `llm:summary` de `Modules/FrontTheme/resources/views/blog/show.blade.php`, `Modules/Directory/resources/views/public/show.blade.php`, `Modules/Dictionary/resources/views/public/show.blade.php` et `Modules/Acronyms/resources/views/public/show.blade.php` - hors périmètre de ce correctif (aucun de ces fichiers n'est la source du défaut sur les fiches d'actualité), signalé plutôt que corrigé sans mandat.
