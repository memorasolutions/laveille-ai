<?php
/**
 * Script PHP web one-shot S95 #237 P27 — Deploy v1.18.3 fix Blade @context corruption JSON-LD.
 * Pattern proc_open (Shell API cpanel KO). Self-delete via @unlink(__FILE__) en fin.
 *
 * Pipeline :
 *   1. git -C / fetch + pull origin master
 *   2. composer dump-autoload --optimize (register app/Helpers/jsonld.php)
 *   3. php artisan view:clear (purge Blade cache — CRITIQUE car compilation @context corrompue persiste sinon)
...
echo "=== DEPLOY v1.18.3 #237 jsonld @context fix ===\n\n";
...
if (function_exists('lv_jsonld_author_stephane')) {
    ...
    echo "Helper lv_jsonld_author_stephane() : OK (Person name=" . ($author['name'] ?? 'KO') . ")\n";
} else {
    echo "Helper lv_jsonld_author_stephane() : KO (non chargé)\n";
}

if (function_exists('lv_jsonld_publisher')) {
    echo "Helper lv_jsonld_publisher() : OK\n";
} else {
    echo "Helper lv_jsonld_publisher() : KO\n";
}
